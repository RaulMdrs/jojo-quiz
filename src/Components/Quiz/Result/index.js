...
const Result = ({ data, step }) => {
    ...
    return (
        <animated.div style={anim}>
            <>
                <section className="flex flex-col gap-3 md:gap-6 items-center justify-center px-8 md:px-32 py-10">
                    <h1 className="text-2xl md:text-4xl text-white uppercase font-semibold md:font-bold md:pb-6">Resultado do seu Quiz</h1>
                    {characterInfo ? (
                        <>
                            <Share toggle={[share, setShare]} text={`Fiz o quiz do Jojo Bizarre Adventure's e meu stand é o ${characterInfo.name}, faça também seu quiz para descobrir o seu acessando: ${window.location.href}`} />
                            ...
                            <h1 className="text-4xl font-bold text-white py-4">{characterInfo.name}</h1>
                    ...
                    <div className="flex flex-col md:flex-row md:justify-between gap-4 pt-6 mt-6 mx-10">
                        <Button restart onClick={() => step(1)}> <span>Reiniciar</span></Button>
                        <Button share onClick={handleShareClick}>
